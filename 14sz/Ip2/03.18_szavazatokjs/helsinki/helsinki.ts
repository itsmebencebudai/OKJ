...
function ObjektumFeltolto(FeldolgozandoAdatok:string[]):HelsinkiAdat[]{
    let AdatObjektum:HelsinkiAdat[]=[];
    for(let i:number=0;i<FeldolgozandoAdatok.length;i++)
    {
        let AdatSor:string[]=FeldolgozandoAdatok[i].split(" ");
        let ujObjektum:HelsinkiAdat={
            helyezes:Number(AdatSor[0]),
            csapatMeret:Number(AdatSor[1]),
            sportag:AdatSor[2],
            versenySzam:AdatSor[3],
        }
        AdatObjektum.push(ujObjektum);
    } 
    return AdatObjektum;
}

let Helsinki=ObjektumFeltolto(helsinki);
