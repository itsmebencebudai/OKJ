...
const uploadUserData = async () => {
  ...
  try {
    ...
    const response = await autFetch(`${import.meta.env.VITE_BASE_API_URL}signup`, {'username':username.value, 'email': email.value, 'password': passwd.value })
  ...
  <div class="signup-container">
    ...
    <form @submit.prevent="uploadUserData">
      ...
      <div style="display: flex; gap: 20px;">
        ...
        <button type="button" class="signup-button" @click="redirectToLogin">Login</button>
      </div>
    </form>
  </div>
</template>
